...
import com.linbo.feature.window.service.MainService;
...
//            if(service.getConfigure().getProps().isEmpty()) {  //判断主类是否已经配置
...
//            }
        ...
        this.addComponent(panel, "main.button.build", new JButton("配置"), button -> {
            button.setPreferredSize(new Dimension(60, 25));
            button.addActionListener(e -> service.enterBuildWindow());
        });
...
//            this.service.switchEditFile(filePath.toString());
        ...
        JMenuItem addExecuteFile = new JMenuItem("添加执行文件");
        addExecuteFile.addActionListener(e -> service.addProjectFile(FileType.EXECUTE_FILE));
        JMenuItem addTestcaseFile = new JMenuItem("添加测试用例文件");
        addTestcaseFile.addActionListener(e -> service.addProjectFile(FileType.TESTCASE_FILE));
        // todo
        JMenuItem deleteItem = new JMenuItem("删除");
        deleteItem.addActionListener(e -> service.deleteProjectFile());
        treePopupMenu.add(addExecuteFile);
        treePopupMenu.add(addTestcaseFile);
    ...
    private JScrollPane createConsole(){
        JTextArea consoleArea = new JTextArea("等待运行... \n");
